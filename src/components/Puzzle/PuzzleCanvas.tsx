...
import { PuzzleCanvasProps } from "./type";
...
export default (comProps: PuzzleCanvasProps) => {
  const props = useMergeProps<PuzzleCanvasProps, keyof typeof defaultProps>(comProps, defaultProps)
  ...
  const {puzzleGridArr, isGameMode, letter, initSpaceIndex, size, grid, isReset, ...context} = useContext(PuzzleCtx)
  const globalAlpha = _globalAlpha;
  const puzzleColor = _puzzleColor;
  const gameModeBackground = _gameModeBackground;
  const puzzleImg = _puzzleImg;
